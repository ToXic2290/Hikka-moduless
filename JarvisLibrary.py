...
В данный момент, модуль загружает нужные библеотеки и предустановки для себя, но вы уже можете им пользоваться.
Джарвис будет работать лишь в тех чатах, в которые вы напишие команду <code>.jaddchat</code>
Для удаления чата, пропишите команду <code>.jremovechat</code>

Вы можете заблокировать пользователя для использования джарвиса командой <code>.zjblockuser (id)</code>
Для разблокировки юзера, введите <code>.junblockuser (id)</code>
...
не стал...
  ...
  help_all = ('''
<b>Список возможностей Джарвиса:

<code>Пинг</code> - вывод скорости отклика джарвиса
<code>Шутка</code> - расскажет рандомную шутку
<code>Кто я</code> - скажет вам кто вы (обидно)
<code>Время</code> - показать время в разных странах
<code>Музыка/трек (название)</code> - найти трек по названию
<code>Аниме</code> - сделает обработку фото в стиле аниме
<code>Тт</code> - скачать видео с ТикТока
<code>Ютуб</code> - скачать видео с ютуба по ссылке</b>
''')
  bot_error = ('''
Упс, сервис перегружен.
Попробуй повторить позже!
''')
  bot_error_format = 'Просто отправь мне картинку!'
